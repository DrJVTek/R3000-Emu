$p = Start-Process -FilePath "E:\Projects\github\Live\R3000-Emu\build_dbg\Debug\r3000_emu.exe" `
    -ArgumentList "--bios=E:\Projects\github\Live\R3000-Emu\bios\ps1_bios.bin","--cd=E:\Projects\github\Live\R3000-Emu\ridgeracer.cue","--fast-boot","--max-steps=50000000","--emu-log-level=info" `
...
Select-String -Path "E:\Projects\github\Live\R3000-Emu\stderr_debug.txt" -Pattern "Fast boot|CORE|EXE|BOOT" | Select-Object -First 20 -ExpandProperty Line
Write-Host "`n=== CRASHES ==="
Select-String -Path "E:\Projects\github\Live\R3000-Emu\stderr_debug.txt" -Pattern "CRASH|fault|Illegal|HALT" | Select-Object -First 10 -ExpandProperty Line
