...
// ── File-static helpers moved to cpu_helpers.h ─────────────────────────────
// gte_func_name / gte_func_is_structural / gte_trace_dump_top (template)
// is_camera_track_addr / is_printable_ascii
// text_flush_line / text_push_char
// reg_name / psx_mmio_name / psx_is_mmio / psx_is_critical_mmio
// ───────────────────────────────────────────────────────────────────────────
...
// record_crash_trace / dump_crash_trace moved to cpu_debug.cpp
...
Cpu::StepResult Cpu::step()
{
    ...
    // HLE BIOS bring-up — body extracted to hle_bios.cpp (Phase 1C-2).
    // handle_hle_bios() returns true when it handled the call (step() must
    // return r immediately) and false in the text-HLE fallthrough case
    // (real BIOS stub must run; continue to fetch-decode).
